...
class OllamaService:
        ...
        for attempt in range(self.max_retries):
            try:
                ollama_request = {
                    ...
                    "stream": stream,
                    "options": {
                        "temperature": 0.7,  # Dengeli yaratıcılık (0-2 arası, 0.7 önerilen)
                        "top_p": 0.9,  # Nucleus sampling (0-1 arası, 0.9 önerilen)
                        "top_k": 40,  # Top-k sampling (daha tutarlı yanıtlar için)
                        "repeat_penalty": 1.1,  # Tekrar cezası (1.0-2.0 arası)
                        "num_predict": 2048,  # Maksimum token sayısı (daha uzun yanıtlar)
                        "stop": ["\n\n\n"]  # Çok fazla boşlukta dur
                    }
                }
    ...
    def generate_stream(self, prompt: str) -> Iterator[str]:
        ...
        Example:
            ...
            >>> for token in service.generate_stream("Hello"):
        ...
        try:
            ollama_request = {
                ...
                "options": {
                    ...
                    "repeat_penalty": 1.1,  # Tekrar cezası
            ...
            for line in response.iter_lines():
                if line:
                    line_str = ""  # Initialize to avoid unbound variable warning
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse streaming response line: {line_str[:100] if line_str else 'empty'}")
                        continue
                    except Exception as e:
